...
class ImageSubscriber(Node):
    ...
    def detect_lines_canny(self, image):
        ...
        if lines is not None:
            ...
            lines = [line for _, line in sorted(zip(lengths, lines), key=lambda x: x[0], reverse=True)]
            lines = lines[:10]
    ...
    def detect_lines(self, image):
        ...
        lines = cv2.HoughLinesP(dog_edges, 
                                rho=1, 
                                theta=np.pi / 180,
                                threshold=180, 
                                minLineLength=120, 
    ...
    def listener_callback(self, data):
        ...
        self.get_logger().info(f'Frame dimensions: {current_frame.shape}')


        # current_frame = self.detect_lines_canny(current_frame)
        current_frame = self.detect_lines(current_frame)
