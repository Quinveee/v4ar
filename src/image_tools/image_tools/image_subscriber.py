...
class ImageSubscriber(Node):
    def __init__(self, topic_name, display_window=False, record_path=None,
        ...
        if oak_info_topic in topics:
            ...
            self.get_logger().warn(
                f"OAK CameraInfo topic {oak_info_topic} not found, skipping OAK rectification")

        # Publisher for processed mono images
    ...
    def load_calibration(self, yaml_file: str):
        try:
            ...
            K, D, P = None, None, None
            if 'camera_matrix' in data:
                K = np.array(data['camera_matrix']['data'],
                             dtype=np.float64).reshape(3, 3)
                D = np.array(data['distortion_coefficients']
                             ['data'], dtype=np.float64).reshape(1, -1)
                if 'projection_matrix' in data:
                    P = np.array(data['projection_matrix']
                                 ['data'], dtype=np.float64).reshape(3, 4)
            elif 'K' in data:
    ...
    def listener_callback(self, data):
        ...
        try:
            ...
            # Publish mono processed image
            if current_frame is not None:
                ros_image = self.br.cv2_to_imgmsg(current_frame)
                self.image_pub.publish(ros_image)

            # -------------------- OAK Rectification -------------------- #
            if self.oak_rect and self.map1_oak is not None:
                if self.use_compressed:
                    np_arr = np.frombuffer(data.data, np.uint8)
                    raw_oak = cv2.imdecode(np_arr, cv2.IMREAD_COLOR)
                else:
                    raw_oak = self.br.imgmsg_to_cv2(data)
