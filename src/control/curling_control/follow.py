...
class MarkerGazeNode(Node):
    def __init__(self):
        ...
        self.declare_parameter("kp_pan", 1)
        self.declare_parameter("kp_tilt", 1)
        ...
        if self.strategy_type not in STRATEGIES:
            self.get_logger().warn(f"Unknown strategy '{self.strategy_type}', defaulting to 'weighted'")
        ...
        self.image_sub = self.create_subscription(Image, "/processed_image", self.image_callback, 1)
        self.marker_sub = self.create_subscription(MarkerPoseArray, "/detected_markers", self.marker_callback, 10)

        # Publish pan and tilt as joint commands
        self.joint_pub = self.create_publisher(JointState, "/ugv/joint_states", 10)

        self.get_logger().info("MarkerGazeNode initialized. Waiting for first image to get dimensions...")
    ...
    def image_callback(self, msg: Image):
        ...
        if self.img_width is None or self.img_height is None:
            ...
            self.get_logger().info(f"Got image dimensions: {self.img_width}x{self.img_height}")
    ...
    def marker_callback(self, msg: MarkerPoseArray):
        ...
        self.get_logger().info(f"Published joint command: pan={pan_angle:.4f}, tilt={tilt_angle:.4f}")
